...
export default function RoadmapApp({ dark = false }) {
  ...
  const serviceCategories = [
    ...
    { id:"ai", name:"AI Services", icon:"🤖", color:T.purple, services:[
      ...
      { name:"CircularReader AI", tier:"Intelligence", desc:"Point to any government circular URL → AI extracts: who is affected, what action is needed, deadline, evidence required.", delivery:["SaaS","API"], revenue:"Included + API credits", phase:"Phase 2" },
      { name:"AuditCopilot AI", tier:"Intelligence", desc:"Assists auditors during evidence review. Highlights anomalies, suggests follow-up questions, drafts finding language.", delivery:["SaaS"], revenue:"Auditor add-on", phase:"Phase 3" },
    ...
    { id:"auditor", name:"Auditor Platform", icon:"🔍", color:T.green, services:[
      ...
      { name:"SamplingEngine", tier:"Professional", desc:"ISAE/ISO-standard random sampling from evidence pool. Statistical confidence calculations.", delivery:["SaaS"], revenue:"Included in Auditor plan", phase:"Phase 2" },
      { name:"ReportForge", tier:"Professional", desc:"Auto-generates audit reports in the exact format for: BCAS, DGCA, NABH, ISO CB, SEBI, RBI, CA Institute.", delivery:["SaaS","API"], revenue:"Per-report credits", phase:"Phase 2" },
      { name:"DigitalCert", tier:"Professional", desc:"Tamper-proof digital audit certificates. Timestamped. Verifiable by regulator via QR code.", delivery:["SaaS"], revenue:"₹500/certificate or subscription", phase:"Phase 3" },
      { name:"CAPATracker", tier:"Professional", desc:"Finding raised → Root cause → Corrective action → Evidence → Closure. Full CAPA loop with deadlines.", delivery:["All models"], revenue:"Included in platform", phase:"Phase 2" },
    ]},
    { id:"api", name:"API & Data", icon:"⚡", color:T.teal, services:[
      { name:"ComplianceScore API", tier:"Data", desc:"Third parties query company X's compliance score. Used by banks for vendor due diligence.", delivery:["API"], revenue:"₹10–50/query or bulk subscription", phase:"Phase 3" },
      { name:"RegulatoryFeed API", tier:"Data", desc:"Real-time feed of all new Indian regulatory circulars, parsed and structured.", delivery:["API"], revenue:"₹10K–50K/month", phase:"Phase 3" },
      { name:"NSWS Bridge", tier:"Integration", desc:"Approval granted on NSWS → auto-creates ongoing compliance calendar in CompliantBharat.", delivery:["SaaS","API"], revenue:"Premium integration", phase:"Phase 4" },
    ...
    { id:"managed", name:"Managed Services", icon:"🛎️", color:T.gold, services:[
      { name:"ComplianceManager", tier:"Managed", desc:"Dedicated compliance analyst maintains your calendar, uploads evidence, prepares audit packs.", delivery:["SaaS + Human"], revenue:"₹25K–1L/month", phase:"Phase 2" },
      { name:"AuditReadiness Sprint", tier:"Managed", desc:"Certification in 60 days? We run a sprint: gap assessment, evidence collection, policy writing, mock audit.", delivery:["SaaS + Services"], revenue:"₹1.5L–10L/engagement", phase:"Phase 2" },
      { name:"VirtualCISO", tier:"Managed", desc:"Senior security expert embedded part-time. Attends board meetings, answers BCAS/DGCA queries.", delivery:["SaaS + Human"], revenue:"₹50K–2L/month", phase:"Phase 3" },
      { name:"TrainingAcademy", tier:"Managed", desc:"Compliance training courses: DPDPA for employees, BCAS AVSEC awareness, ISO 27001 fundamentals.", delivery:["SaaS LMS"], revenue:"₹500–2,000/seat", phase:"Phase 2" },
    ]},
    { id:"govt", name:"Regulator Layer", icon:"🏛️", color:T.navy, services:[
      { name:"RegulatorDashboard", tier:"Government", desc:"Sector-wide compliance health. SEBI sees all 5,000 brokers' scores. BCAS sees all 157 airports.", delivery:["Private Cloud","On-Prem"], revenue:"MoU / grant / subscription", phase:"Phase 4" },
      { name:"CircularDistribution", tier:"Government", desc:"Regulators issue circulars THROUGH the platform → auto-distributed to all affected entities.", delivery:["Private Cloud"], revenue:"Government licensing", phase:"Phase 4" },
      { name:"InspectionQueue AI", tier:"Government", desc:"AI prioritises which entities to inspect next. Lowest scores + approaching deadlines = inspect first.", delivery:["Private Cloud"], revenue:"Government contract", phase:"Phase 4" },
      { name:"StateGovPortal", tier:"Government", desc:"White-labelled portal for state governments. One MoU → 50,000 MSMEs onboarded.", delivery:["Private Cloud"], revenue:"₹1–5Cr/year per state", phase:"Phase 4" },
    ]},
    { id:"marketplace", name:"Marketplace", icon:"🛒", color:T.accent, services:[
      { name:"ConsultantMarketplace", tier:"Platform", desc:"Verified ISO consultants, CA firms listed. Business needs ISO 27001 consultant? Browse, compare, book.", delivery:["SaaS Marketplace"], revenue:"10–15% commission", phase:"Phase 3" },
      { name:"CertificationBodyConnect", tier:"Platform", desc:"Book ISO certification audit directly. BIS, DNV, Bureau Veritas, TUV listed.", delivery:["SaaS Marketplace"], revenue:"Referral fee from CBs", phase:"Phase 3" },
      { name:"InsuranceConnect", tier:"Platform", desc:"Cyber insurance providers check your ComplianceScore to price policies. High score = lower premium.", delivery:["SaaS + API"], revenue:"Commission per policy", phase:"Phase 4" },
    ]},
  ];
  ...
  return (
    <div style={{ fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif", background:T.bg, minHeight:"100vh", color:T.text }}>
      ...
      {/* HEADER */}
      <div className="avia-hdr" style={{ background:T.surface, borderBottom:`1px solid ${T.border}`, boxShadow: dark ? "0 1px 16px rgba(0,0,0,0.4)" : "0 1px 12px rgba(26,86,219,0.06)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div>
            <div style={{ fontSize:18, fontWeight:800, color:T.text, letterSpacing:"-0.02em", lineHeight:1 }}>CompliantBharat</div>
            <div style={{ fontSize:10, color:T.muted, marginTop:2, letterSpacing:"0.04em" }}>Platform Vision & Roadmap</div>
          </div>
          <div style={{ marginLeft:"auto", display:"flex", gap:6, flexShrink:0 }}>
            <div style={{ padding:"4px 10px", borderRadius:20, background:T.blueL, fontSize:10, fontWeight:700, color:T.blue, border:`1px solid ${T.blue}44` }}>Regulatory</div>
            ...
            <div style={{ padding:"4px 10px", borderRadius:20, background:T.amberL, fontSize:10, fontWeight:700, color:T.accent, border:`1px solid ${T.accent}44` }}>Gold</div>
          </div>
        </div>
      </div>
